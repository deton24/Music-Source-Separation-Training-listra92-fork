...
class BSRoformer(Module):
    ...
    def __init__(
            ...
            *,
            ...
            use_pope: bool = False,
            inference_stem_batch_size: int = 0,
            inference_offload_mask_estimators: bool = False,
    ):
        super().__init__()
        ...
        self.inference_stem_batch_size = max(inference_stem_batch_size, 0)
        self.inference_offload_mask_estimators = inference_offload_mask_estimators
    ...
    def _reconstruct_from_masks(
        ...
        stft_with_masks = stft_repr * masks
        stft_with_masks = rearrange(stft_with_masks, 'b n (f s) t -> (b n s) f t', s=self.audio_channels)

        if self.zero_dc:
            stft_with_masks = stft_with_masks.index_fill(1, tensor(0, device=device), 0.)
        ...
        try:
            ...
            recon_audio = torch.istft(
                stft_with_masks.cpu() if x_is_mps else stft_with_masks,
                **self.stft_kwargs,
    ...
    @staticmethod
    def _move_heads_to_device(heads: List[Module], device: torch.device):
        for head in heads:
            head.to(device)

    @staticmethod
    def _move_heads_to_cpu(heads: List[Module]):
        for head in heads:
            head.to('cpu')
    ...
    def forward(
        ...
        use_mask_offload = (
            not exists(target)
            and self.inference_offload_mask_estimators
            and device.type == 'cuda'
        )

        use_stemwise_mask_processing = (
            not exists(target)
            and (self.inference_stem_batch_size > 0 or use_mask_offload)
        )

        if use_stemwise_mask_processing:
            if use_mask_offload:
                self._move_heads_to_cpu(list(self.mask_estimators))
                torch.cuda.empty_cache()

            chunk_size = self.inference_stem_batch_size if self.inference_stem_batch_size > 0 else 1
            chunk_size = min(chunk_size, num_stems)
            recon_audio_chunks = []
            for start_idx in range(0, num_stems, chunk_size):
                chunk_heads = heads[start_idx:start_idx + chunk_size]
                if use_mask_offload:
                    self._move_heads_to_device(chunk_heads, device)
                if self.use_torch_checkpoint:
                    chunk_masks = torch.stack([checkpoint(fn, x, use_reentrant=False) for fn in chunk_heads], dim=1)
                else:
                    chunk_masks = torch.stack([fn(x) for fn in chunk_heads], dim=1)
                chunk_masks = rearrange(chunk_masks, 'b n t (f c) -> b n f t c', c=2)
                chunk_audio = self._reconstruct_from_masks(
                    stft_repr=stft_repr,
                    masks=chunk_masks,
                ...
                chunk_num_stems = len(chunk_heads)
                chunk_audio = rearrange(
                    chunk_audio,
                    '(b n s) t -> b n s t',
                ...
                if use_mask_offload:
                    self._move_heads_to_cpu(chunk_heads)
                recon_audio_chunks.append(chunk_audio)
            recon_audio = torch.cat(recon_audio_chunks, dim=1)
        else:
            if self.use_torch_checkpoint:
            ...
            recon_audio = self._reconstruct_from_masks(
                stft_repr=stft_repr,
                masks=masks,
                stft_window=stft_window,
                audio_length=raw_audio.shape[-1],
